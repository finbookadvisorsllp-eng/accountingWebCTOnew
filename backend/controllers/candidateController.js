...
exports.submitInterestForm = async (req, res) => {
  try {
    ...
    if (!personalInfo || !personalInfo.firstName || !personalInfo.primaryContact?.number) {
...
exports.submitExitedForm = async (req, res) => {
  try {
    ...
    if (candidateId) {
      ...
      if (candidate.status !== 'INTERESTED' && candidate.status !== 'ALLOWED_EXITED') {
        return res.status(400).json({
          success: false,
      ...
      candidate.exitedDocuments = { ...candidate.exitedDocuments, ...exitedDocuments };
  ...
  try {
    ...
    if (search) {
      query.$or = [
        ...
        { 'personalInfo.primaryContact.number': { $regex: search, $options: 'i' } },
...
exports.approveCandidate = async (req, res) => {
  try {
    ...
    if (adminData) {
      candidate.employeeContactInfo = adminData.employeeContactInfo || candidate.employeeContactInfo;
      candidate.contractInfo = adminData.contractInfo || candidate.contractInfo;
      candidate.legalCompliance = adminData.legalCompliance || candidate.legalCompliance;
    }
...
exports.updateAdminFields = async (req, res) => {
  try {
    ...
    if (employeeContactInfo) candidate.employeeContactInfo = { ...candidate.employeeContactInfo, ...employeeContactInfo };
    if (contractInfo) candidate.contractInfo = { ...candidate.contractInfo, ...contractInfo };
    if (legalCompliance) candidate.legalCompliance = { ...candidate.legalCompliance, ...legalCompliance };
