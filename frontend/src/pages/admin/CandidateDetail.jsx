...
import { FaArrowLeft, FaCheck, FaTimes, FaUser, FaEnvelope, FaPhone, FaMapMarkerAlt } from 'react-icons/fa';
...
const CandidateDetail = () => {
  ...
  const handleAllowExited = async () => {
    if (!window.confirm('Allow this candidate to fill the exited form?')) return;
    ...
    return (
      <AdminLayout>
        <div className="text-center py-12">
          ...
          <Link to="/admin/candidates" className="text-primary-600 hover:text-primary-700 mt-4 inline-block">
      ...
      <div className="space-y-6">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="font-semibold">{candidate.adminInfo?.employeeId || 'Not Generated'}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-lg">
            <h2 className="text-xl font-bold text-gray-900 mb-4 border-b pb-2">Personal Information</h2>
            <div className="space-y-3">
              ...
              <div>
                ...
                <p className="font-semibold">
                  {candidate.personalInfo?.dateOfBirth ? new Date(candidate.personalInfo.dateOfBirth).toLocaleDateString() : 'N/A'}
                </p>
              </div>
              <div>
              ...
              {candidate.exitedPersonalInfo?.maritalStatus && (
                <div>
                  ...
                  <p className="font-semibold">{candidate.exitedPersonalInfo.maritalStatus}</p>
                </div>
              )}
              {candidate.exitedPersonalInfo?.nationality && (
                <div>
                  <p className="text-sm text-gray-600">Nationality</p>
                  <p className="font-semibold">{candidate.exitedPersonalInfo.nationality}</p>
                </div>
              )}
            </div>
          </div>
                  ...
                  <div>
                    ...
                    <ul className="list-disc list-inside">
                      {candidate.education.certifications.map((cert, index) => (
                        <li key={index} className="font-semibold">{cert}</li>
                      ))}
                    </ul>
                  </div>
                )}
        ...
        {candidate.detailedEducation?.length > 0 && (
          <div className="bg-white rounded-2xl p-6 shadow-lg">
            <h2 className="text-xl font-bold text-gray-900 mb-4 border-b pb-2">Detailed Education</h2>
                ...
                <div key={index} className="bg-gray-50 p-4 rounded-lg">
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="font-semibold">{edu.yearOfPassing} - {edu.percentage}</p>
                    </div>
                  </div>
                </div>
              ))}
        ...
        {candidate.detailedWorkExperience?.length > 0 && (
          <div className="bg-white rounded-2xl p-6 shadow-lg">
            <h2 className="text-xl font-bold text-gray-900 mb-4 border-b pb-2">Work Experience</h2>
            <div className="space-y-4">
        ...
        {candidate.professionalInterests && (
          <div className="bg-white rounded-2xl p-6 shadow-lg">
            <h2 className="text-xl font-bold text-gray-900 mb-4 border-b pb-2">Professional Interests</h2>
            <div className="space-y-4">
              {candidate.professionalInterests.whyJoinTeam && (
                <div>
                  <p className="text-sm text-gray-600">Why Join Our Team</p>
                  <p className="font-semibold">{candidate.professionalInterests.whyJoinTeam}</p>
                </div>
              )}
              {candidate.professionalInterests.longTermGoals && (
                <div>
                  <p className="text-sm text-gray-600">Long-term Goals</p>
                  <p className="font-semibold">{candidate.professionalInterests.longTermGoals}</p>
                </div>
              )}
              {candidate.professionalInterests.availabilityToJoin && (
                <div>
                  <p className="text-sm text-gray-600">Availability</p>
                  <p className="font-semibold">{candidate.professionalInterests.availabilityToJoin}</p>
                </div>
              )}
            </div>
          </div>
      ...
      {showApproveModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-8 max-w-md w-full">
            <h2 className="text-2xl font-bold text-gray-900 mb-4">Approve Candidate</h2>
            <div className="space-y-4">
              <div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setApproveData({ ...approveData, dateOfJoining: e.target.value })}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-600 focus:border-transparent transition"
                />
              </div>
            </div>
